...
<?php foreach ($tracks as $i => $url) { ?>
		<div id="content">Individual <?php echo $i ?>: <span name="rate<php echo $i?>"><?php 
		if ($ratings[$i]) { ?>Thanks for rating!<?php } else { foreach (array(1,2,3,4,5) as $vote) { ?><a href="" name="rate<?php echo $i; ?>-<?php echo $vote; ?>"><img src="rate<?php echo $vote; ?>.png" alt="Rate <?php echo $vote; ?> points for individual <?php echo $i ?>" height="16"/></a> <?php } } ?></span><a class="soundcloud-player" id="basic" href="<?php echo $url; ?>">Play</a><br/><?php if ($i != 0) { ?><hr/><?php } ?></div>
<?php } ?>
